...
x_aug, y_aug = gan_trainer.train(x_train, y_train, epochs=2000)
x_train = np.vstack((x_train, x_aug))
y_train = np.vstack((y_train, y_aug))

x_train = savgol_filter(x_train, window_length=5, polyorder=2, axis=1)
x_test = savgol_filter(x_test, window_length=5, polyorder=2, axis=1)
...
pls_models = [
    ('PLS-15', PLSRegression(n_components=19)),
]
